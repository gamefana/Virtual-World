    ...
    */
    ...
    private void loadImages(String filename, ImageStore imageStore,
        ...
        try {
            ...
            ImageProcessor.loadImages(in, imageStore, screen);
    ...
    private void loadWorld(WorldModel world, String filename,
        ...
        try {
            Scanner in = new Scanner(new File(filename));
            LineProcessor.load(in, world, imageStore);
        } catch (FileNotFoundException e) {
            System.err.println(e.getMessage());
        }
    }
    ...
    private void scheduleActions(WorldModel world,
        ...
        for (Entity entity : world.getEntities()) {
            if (entity.accept(new AllFalseEntityVisitor(){
                @Override
                public Boolean visit(Ore ore){return true;}
                public Boolean visit(OreBlob blob){return true;}
                public Boolean visit(MinerFull miner){return true;}
                public Boolean visit(MinerNotFull miner){return true;}
                public Boolean visit(Quake quake){return true;};
                public Boolean visit(Vein vein){return true;}
            }))
                ((ActivableEntity)entity).scheduleActions(scheduler, world, imageStore);
        }
    }
